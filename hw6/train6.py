...
#exit(0)
# preprocess data
x_data = pre(sys.argv[1])
y_data = pd.read_csv(sys.argv[2])['label'].values
N = 119018
x_data = x_data[:N]
y_data = y_data[:N]
...
save_best_model=ModelCheckpoint(best_weights_filepath, monitor='loss', save_best_only=True, mode='auto')
name=sys.argv[3]
try:
    model.fit(x_data,y_data,callbacks=[early_stopping, save_best_model], batch_size=1200,epochs=30)
except:
    model.save(name)
