...
if len(sys.argv) == 3:
    ...
    model = Sequential()
    model.add(emb)
    model.add(Bidirectional(GRU(units=512, return_sequences=True, dropout=0.2, recurrent_dropout=0.2)))
    model.add(Dropout(0.4))
    model.add(Bidirectional(GRU(units=256, return_sequences=True, dropout=0.2, recurrent_dropout=0.2)))
    model.add(Dropout(0.4))
...
try:
    ...
    model.save(datetime.datetime.now().strftime("%m-%d-%H:%M:%S") + '-rnn7-crashed.h5')
model.load_weights(best_weights_filepath)
result = model.evaluate(x_test,y_test,batch_size=1200)
print(result)
accuracy = result[1]
model.save(datetime.datetime.now().strftime("%m-%d-%H:%M:%S-") + str(accuracy) + '-rnn7.h5')
